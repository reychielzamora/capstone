...
try {
    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
        ...
        $avatarFilename = 'avatar_' . $official_id . '_' . time() . '.' . $avatarExt;
        $avatarFullPath = $avatarDir . $avatarFilename;

        if (move_uploaded_file($avatar['tmp_name'], $avatarFullPath)) {
            if (!empty($old_pp) && file_exists(__DIR__ . '/' . $old_pp)) {
                unlink(__DIR__ . '/' . $old_pp);
            }
    ...
    if (isset($_FILES['signature']) && $_FILES['signature']['error'] === UPLOAD_ERR_OK) {
        ...
        $newFilename = 'official_' . $official_id . '_' . time() . '.' . $ext;
